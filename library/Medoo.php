...
class Medoo
{
	...
	protected $_distinct_model = false;
...
//		array_push($this->logs, $query);
			...
			else
			{
				...
				if (isset($match[4]))
				{
					...
					if ($operator == '|' || $operator == '&')
					{
						...
						{
							$logic = array_keys($value);
							$logic = $logic[0];
							$tmp = [];
							foreach($value[$logic] as $v) {
								$tmp[] = "{$column} {$operator} $v";
							}
							$wheres[] = implode(" {$logic} ", $tmp);
						}
					}
				}
				else
	...
	protected function select_context($table, $join, &$columns = null, $where = null, $column_fn = null)
	{
		...
		if($this->_distinct_model && $columns != '*'){
			$distinct = "DISTINCT ";
		}else{
			$distinct = '';
		}
